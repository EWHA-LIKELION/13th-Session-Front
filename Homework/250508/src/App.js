...
function App() {
  ...
  const [selectedLionId, setSelectedLionId] = useState(null);
  const [selectedLionName, setSelectedLionName] = useState("");
  useEffect(() => {
    const selectedLion = trackData.find((lion) => lion.id === selectedLionId);
    setSelectedLionName(selectedLion ? selectedLion.name : "");
  }, [trackData, selectedLionId]); // trackData나 selectedLionId가 변경될 때마다 실행
