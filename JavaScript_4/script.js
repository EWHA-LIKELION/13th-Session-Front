...
const printTodoItem = (text) => {
  ...
  todoSpan.addEventListener("click", toggleTodoToDone);
  // [할 일 삭제 버튼]
...
const printDoneItem = (text) => {
  ...
  doneSpan.addEventListener("click", toggleDoneToDo);

  // [끝낸 일 삭제 버튼]
  // 생성한 button 태그에 "삭제"라는 텍스트를 넣어주세요.
  doneBtn.innerText = "삭제";

  // 생성한 button 태그를 클릭하면 deleteDoneItem 함수가 실행되게 해주세요.
  doneBtn.addEventListener("click", deleteDoneItem);
